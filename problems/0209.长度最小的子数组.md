...
* 不需要“回头”，因为当进入内部的 while 循环时，当前的 left 指针已经不可能找到比当前更短的子数组了，因为对于相同的 right 指针，再往左移动 left 指针只会使得子数组的长度增加，而不会使得子数组的和更接近目标值 s。因此，通过逐步右移 right 指针，对于每个 right 指针，只需要从上一个 left 指针的位置开始搜索，而不需要回到更左边的位置重新搜索，这样可以确保每个子数组都是以最短的长度满足条件的
...
public:
    int minSubArrayLen(int target, vector<int>& nums) {
        ...
        if(ans == INT_MAX) return 0;  // 如果result没有被赋值的话，就返回0，说明没有符合条件的子序列
        return ans;
    }
};
```
