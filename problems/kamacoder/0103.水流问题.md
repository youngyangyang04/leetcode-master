...
public class Main {
    static boolean mark1 = false; // 记录该格子的水能否到达第一组边界
    static boolean mark2 = false; // 第二组边界
    static int[][] dir = {{0,1},{0,-1},{-1,0},{1,0}};
    static List<List<Integer>> result = new ArrayList<>();  // 记录每个符合条件的格子坐标
    public static void main(String[] args) {
        ...
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < m; j++) {
                mark1 = false; // 初始化标记
                mark2 = false;
                int[][] visited = new int[n][m]; // 记录在当前格子出发的水流中某个格子是否被流经
                dfs(grid, visited, i, j); // 深搜模拟水流的流动
                if (mark1 && mark2) { // 如果满足条件即可收获结果
    ...
    public static void dfs(int[][] grid, int[][] visited, int x, int y) {
        visited[x][y] = 1; // 记录格子被流经
        if (x == 0 || y == 0) { // 如果流经的格子满足第一组或第二组边界条件就可以把标记置为true
            mark1 = true;
        }
        if (x == grid.length - 1 || y == grid[0].length - 1) { // 同上
            mark2 = true;
        }
        for (int i = 0; i < 4; i++) { // 模拟水流向四周流动
            int nextX = x + dir[i][0];
            int nextY = y + dir[i][1];
            if (nextX < 0 || nextX >= grid.length || nextY < 0 || nextY >= grid[0].length) {
                continue; // 邻居下标越界直接continue
            }
            if (grid[x][y] >= grid[nextX][nextY] && visited[nextX][nextY] == 0) { // 必须当前坐标高度大于等于邻居高度才能流动，同时邻居必须没有被流经过，避免出现套娃死循环
                dfs(grid, visited, nextX, nextY); // 继续深搜
            }
        }
    }
}
