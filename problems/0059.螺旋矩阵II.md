...
​	想象有一个小人，他只能向上、右、下、左四个方向前进，当碰到墙壁或者“已占领”的区域时，他就更换前进的方向，最终走完整个二维数组，用动图表示如下：

![GIF 2022-1-6 14-41-35](https://image-static.segmentfault.com/261/428/2614281176-61d69488e14b7)
...
function generateMatrix(n: number): number[][] {
    ...
    let curDirIndex: number = 0;
    const resArr: number[][] = new Array(n).fill(1).map(i => new Array(n).fill(-1));
    let x: number = 0, y: number = 0;
    // walk arr and put value
    for (let i = 0; i < n * n; i++) {
        resArr[y][x] = i + 1;
        let tempX: number = x + dirMap[curDirIndex][0];
        let tempY: number = y + dirMap[curDirIndex][1];
        if (tempX >= n || tempX < 0 || tempY >= n || tempY < 0 || resArr[tempY][tempX] !== -1) {
            // change direction
            curDirIndex = (curDirIndex + 1) % 4;
        }
        x += dirMap[curDirIndex][0];
        y += dirMap[curDirIndex][1];
    }
    return resArr;
};
```
