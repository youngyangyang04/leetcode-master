...
即从左向右遍历：maxLeft[i] = max(height[i - 1], maxLeft[i - 1]);

从右向左遍历：maxRight[i] = max(height[i + 1], maxRight[i + 1]);
...
public:
    int trap(vector<int>& height) {
        ...
        // 记录每个柱子左边柱子最大高度
        maxLeft[0] = 0; // 下标为0的柱子左边没有柱子，所以为0
        for (int i = 1; i < size; i++) {
            maxLeft[i] = max(height[i - 1], maxLeft[i - 1]);
        }
        // 记录每个柱子右边柱子最大高度
        maxRight[size - 1] = 0; // 同理最右侧的柱子右边没有柱子，所以为0
