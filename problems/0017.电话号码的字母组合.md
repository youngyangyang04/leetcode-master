...
class Solution {
	...
	private void backTrace(String[] map, char[] digit, int start) {
		//如果原串为""或者当前组合符合要求则加入到ret中
		if(sb.length() == digit.length && !sb.toString().equals("")) {
			ret.add(sb.toString());
			return;
		}
		//以"23"为例
		...
		for(int i=start;i<digit.length;i++) {
			//遍历每个digit对应的字母 2 ->"abc" 遍历a..b..c
			for(int j=0;j<map[digit[i]-'0'].length();j++) {
				sb.append(map[digit[i]-'0'].charAt(j));
				backTrace(map, digit, i+1);
