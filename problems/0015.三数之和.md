...
本题中A，B，C指针的移动都有以下两种来源修改都有以及去重两种来源

- A，B，C 三元组匹配后 ++ / --
- 去重

循环次数不确定，因此比起常用在固定循环次数的for，用while会更清晰

而且若去重条件为`nums[cur] == nums[last]`，即对相同的数值，第一次进行匹配，之后的情况要被去重
因此无论A，B，C 的代码逻辑都应为

- 匹配：第一次取到某个数值，进行匹配
- 移动：调整下标（可能第二次取到相同的数值）
- 去重：如果第二次取到相同的数值，去重 
- 检测：去重使指针移动后是否到达终止条件，检测循环是否继续运行
