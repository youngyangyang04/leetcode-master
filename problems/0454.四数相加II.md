...
#### 版本二（二分做法）
* 时间复杂度: O(n^2 logn)
* 空间复杂度: O(n^2) 存储的两个List数组的空间
* 代码思路：将四个数组，两两组合分成两个组，然后排序，第一层for循环遍历第一个list，中间嵌套两个二分，第一个二分是寻找答案的左边界，第二个二分是寻找答案的右边界，累加二分的区间即为答案。
...
class Solution {
    public int fourSumCount(int[] nums1, int[] nums2, int[] nums3, int[] nums4) {
        ...
        for (int i = 0; i < list.size(); i ++ )
        {
            ...
            if (list1.size() == 0 || value < list1.get(0) 
            || value > list1.get(list1.size() - 1)) continue;
            // 上面的代码确保了答案一定在其中，不在其中的已经continue掉了
            //寻找答案左边界
            while (l < r) {
                int mid = (l + r) / 2;
                if (list1.get(mid) >= value) r = mid;
                else l = mid + 1;
            }
