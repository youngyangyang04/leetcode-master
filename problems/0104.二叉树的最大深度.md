...
int maxDepth(struct Node* root) {
    ...
    for (int i = 0; i < root->numChildren; i++) {
        int curr_max = maxDepth((root->children)[i]);
...
int maxDepth(struct Node* root) {
    ...
    // initialise queue
    int q_len = 100;
    struct Node **q = (struct Node **)malloc(q_len * sizeof(struct Node *));
    assert(q);
    int head = 0, tail = 0;
    q[tail++] = root;
    ...
    while (level_size = tail - head) {
        for (int i = 0; i < level_size; i++) {
            ...
            for (int j = 0; j < temp->numChildren; j++) {
                if (tail == q_len) {
                    q_len *= 2;
                    q = (struct Node **)realloc(q, q_len * sizeof(struct Node *));
                    assert(q);
                }
                q[tail++] = (temp->children)[j];
            }
        }
        head += level_size;
        depth += 1;
