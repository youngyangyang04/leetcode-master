...
### Python
```python3
# 方法一：递归法，从上往下换（广度+深度结合版），从下往上换（深度优先）
class Solution:
    def invertTree(self, root: TreeNode) -> TreeNode:
        if not root:
            return None
            
        root.left, root.right = root.right, root.left    # 交换在最前，就是从上往下换节点
        self.invertTree(root.left)    
        self.invertTree(root.right)
        # root.left, root.right = root.right, root.left    # 交换在最后，就是从下往上换节点
        return root
```

使用迭代法，从根节点往下第一步就会交换左右节点，如果是深度优先应该严格遵循从最左节点走到底进行交换再往上返回。
所以这里写成深度+广度的结合版本，而不是传统意义上的深度优先。
```python3
# 方法二：迭代法，广度+深度结合版（层序遍历），使用栈的做法。
class Solution:
    def invertTree(self, root: TreeNode) -> TreeNode:
        if not root:
            return root
        ...
        while st:
            ...
            # 因为这是交换节点不是打印轨迹，先加右边或左边都可以，不用严格按照前序遍历走，不理解可以画画图感受一下
            if node.right:
...
```python
# 方法三：迭代法，广度+深度结合版（层序遍历），使用队列的做法。可以用list模拟队列
