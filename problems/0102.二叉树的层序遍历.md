...
<img src="0102.二叉树的层序遍历.assets/image-20210606110240273.png" alt="image-20210606110240273" style="zoom:80%;" />



思路：

沿用固定套路，只需要添加一个指示层数的变量即可，偶数层顺序不变，奇数层反转顺序。
...
class Solution:
    def levelOrder(self, root: TreeNode) -> List[List[int]]:
        ...
        while quene:
            ...
            out_list.append(in_list if not depth%2 else list(reversed(in_list)))
            depth += 1

        return out_list

# 执行用时：36 ms, 在所有 Python3 提交中击败了92.18%的用户
# 内存消耗：15.2 MB, 在所有 Python3 提交中击败了41.16%的用户   
```
