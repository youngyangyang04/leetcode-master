...
```python3
# 优化方案，使用deque进一步优化时间复杂度，用数组模拟queue每次在pop(0)的时候时间复杂度为O(n)
...
class Solution:
    def rightSideView(self, root: TreeNode) -> List[int]:
        def get_val_and_append(add=False):
            node = queue.popleft()
            if node.left:
                queue.append(node.left)
        ...
        while queue:
	    # deque也可以通过queue[-1]取最后一个元素，这里为了统一写法没有这么操作
            for i in range(len(queue) - 1):
                get_val_and_append()
            get_val_and_append(True)
        return res
...
题中如root.children 是一个地址的list，我们在用queue的时候本身就是一个list了
如果使用append的话，则会出现在序列里计算序列的情况，会报错，我们希望得到每一次len(queue)
的话，应该在加入的时候把children展开
...
class Solution:
    def largestValues(self, root: TreeNode) -> List[int]:
        ...
        return list(map(lambda x: max(x), res))
```
java代码：
...
```python3
# 方法一：104. 层序遍历法解答
# 和普通层序遍历一样，只是这次完成一次队列的遍历，那么depth就要+1
    ...
    def maxDepth(self, root: TreeNode) -> int:
        ...
        while queue:
            for i in range(len(queue)):
                ...
                if node.right:
                    queue.append(node.right)
            depth += 1    # 完成一次queue的for循环就+1，因为说明这一层有节点。
        return depth
...
```python3
# 方法二：104. 深度优先法解答
# 利用递归，每次返回的时候选择左右子树中深度更大的那个+1（即加上本层的深度）
...
class Solution:
    def maxDepth(self, root: TreeNode) -> int:
        ...
        left_hight = self.maxDepth(root.left)   # 记录左子树的最大深度
        right_height = self.maxDepth(root.right)    # 记录右子树的最大深度

        return max(left_hight, right_height) + 1    # 从最大的那个取一个加一即是当前树的最大深度
```
...
class Solution:
    def minDepth(self, root: TreeNode) -> int:
        ...
        if not root:
            return 0
        elif not root.left:     # 如果没有左子，就以右边的为准
            return self.minDepth(root.right) + 1
        elif not root.right:    # 同理没有右子，就以左边为准
    ...
    def minDepth(self, root: TreeNode) -> int:
        ...
        while queue:
            for i in range(len(queue)):
                ...
                if not node.left and not node.right:  # 如果左右都没有了，说明到达叶节点了
                    return depth + 1                  # 返回当前深度+1
            depth += 1
        return depth
