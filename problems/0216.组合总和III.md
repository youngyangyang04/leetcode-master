...
class Solution {
    ...
    public void backtracking (int k, int targerSum, int startIndex) {
        // 值已经大于目标值后序就无意义了，直接返回
        if (targerSum < 0) {
            return;
        }
        // k == 0表示已经达到所要求的组合所含元素数量了，返回
        if (k == 0) {
            // 组合所含元素数量满足且和也满足，接估计添加该条路径
            if (targerSum == 0) {
        ...
        for (int i = startIndex; i <= 9 - k + 1; i++) {
            path.add(i);
            // 所要求元素数量-1，目标和减去当前添加的元素值，起始下标也加1
            backtracking(k - 1, targerSum - i, i + 1);
            // 回溯
