...
class Solution:
    def detectCycle(self, head: ListNode) -> ListNode:
        ...
        while True:
            ...
            if fast == slow:       # 当快慢指针相遇， 说明链表中存在环， 退出 while；
                break
            if not fast and  fast.next:  # 此处表明 快指针走到链表末尾， 都没有遇到过 慢指针， 说明链表中就没有环，
              return  None

        ptr = head          #  有数学分析可知，   当 快指针走完一圈后， 遇到慢指针时
        meet_node = slow     #   此时，从环内相遇节点处 走到 环入口处的距离 =  从头节点处 走到 环入口处的距离
        while ptr != meet_node:
            ptr = ptr.next
            meet_node = meet_node.next

        enter_node = ptr         #   当循环结束时， 此时ptr 所到达的位置 = 便是环的入口处；
        return  enter_node
