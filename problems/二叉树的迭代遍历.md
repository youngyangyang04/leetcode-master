...
type Mutable<T> = Option<Rc<RefCell<T>>>;

#[derive(Debug)]
struct TreeNode {
    val: i32,
    left: Mutable<TreeNode>,
    right: Mutable<TreeNode>,
}
...
impl Solution {
    /// 前序遍历-迭代版
    fn preorder_tranversal(root: Mutable<TreeNode>) -> Vec<i32> {
        if root.is_none() {
            return vec![];
        }
    ...
    /// 中序遍历-迭代版
    fn inorder_traversal(root: Mutable<TreeNode>) -> Vec<i32> {
        if root.is_none() {
            return vec![];
        }
    ...
    /// 后续遍历-迭代版
    fn postorder_traversal(root: Mutable<TreeNode>) -> Vec<i32> {
        if root.is_none() {
            return vec![];
        }
