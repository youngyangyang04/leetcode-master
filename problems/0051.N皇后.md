...
class Solution:
    def solveNQueens(self, n: int) -> List[List[str]]:
        ans, path = [], []
        # board = [["."]*n]*n ### Fail 这种初始化方法不行！
        # board = [["."]*n for i in range(n)] ### Succeed
        # board = ["." for i in range(n)] * n ### Succeed
        board = [["." for i in range(n)] for j in range(n)] ### Succeed

        def isvalid(r, c):
            for i in range(r):
                if board[i][c] == "Q": return False # | 判断正上方 c=j
                if i - r + c >= 0 and board[i][i-r+c] == "Q": return False # \ 判断左上角 c-j=r-i
                if -i + r + c <= n-1 and board[i][-i+r+c] == "Q": return False # / 判断右上角 j-c=r-i
            return True
        ...
        def backtrack(r):
            for c in range(n):
                ...
                board[r][c] = "Q"
                path.append("".join(board[r])) # 将r行的board信息存入path
                if r == n-1:
