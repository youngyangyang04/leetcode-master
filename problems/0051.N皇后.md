...
class Solution:
    def solveNQueens(self, n: int) -> List[List[str]]:
        ...
        ret = []
        self.backtracking(ret, chessboard, n, 0)
        return ret

    def backtracking(
        self, ret: List[List[str]], chessboard: List[List[str]], n: int, row: int
    ):
        if row == n:
            ret.append(["".join(row) for row in chessboard])
            return
        for j in range(n):
            if self.is_valid(chessboard, n, row, j):
                chessboard[row][j] = "Q"
                self.backtracking(ret, chessboard, n, row + 1)
                chessboard[row][j] = "."

    def is_valid(self, chessboard: List[List[str]], n: int, row: int, col: int) -> bool:
        # check column |
        for i in range(row):
            if chessboard[i][col] == "Q":
                return False
        ...
        # check diagonal \ and /
        directions = [(-1, -1), (-1, 1)]  # check above rows only
...
impl Solution {
    pub fn solve_n_queens(n: i32) -> Vec<Vec<String>> {
        ...
        Self::backtracking(&mut ret, &mut chessboard, n, 0);
        ret
    }
    ...
    fn backtracking(
        ...
        if row == n {
            ret.push(chessboard.iter().map(|row| row.iter().collect()).collect());
            return;
        }
        for j in 0..n {
            if Self::is_valid(chessboard, n, row, j) {
    ...
    fn is_valid(chessboard: &mut Vec<Vec<char>>, n: usize, row: usize, col: usize) -> bool {
        // check column |
        for i in 0..row {
            if chessboard[i][col] == 'Q' {
                return false;
        ...
        // check diagonal \ and /
        let directions = [(-1, -1), (-1, 1)]; // check above rows only
        for (dr, dc) in directions {
            let mut i = row as isize + dr;
            let mut j = col as isize + dc;
            while (0 <= i && i < n as isize) && (0 <= j && j < n as isize) {
