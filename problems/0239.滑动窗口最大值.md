...
void QueuePop(MyQueue* que, int value, int k){
    // 如果队列不为空且队头为value, 则把队头弹出去, 这里只维护一个大小为k的队列
    if(que->head < que->tail && que->q[que->head%k]==value){
        que->head++;
    }
}

void QueuePush(MyQueue* que, int value, int k){
    // 如果队列不为空且队尾小于value, 把队尾弹出去, 保证队列是从大到小的
    while(que->head < que->tail && que->q[(que->tail-1)%k] < value){
        que->tail--;
    }
    // 当tail超出k之后, 就会去覆盖队列的最开头
    que->q[que->tail%k] = value;
    que->tail++;
}
// 因为队列是从大到小的, 所以队头就是最大的值
int QueueFront(MyQueue* que, int k){
    return que->q[que->head%k];
}

int* maxSlidingWindow(int* nums, int numsSize, int k, int* returnSize){
    MyQueue* que = (MyQueue*)malloc(sizeof(MyQueue));
    // 只维护一个大小为k的队列, 通过head%k跟tail%k来查找值
    que->q = (int*)malloc(sizeof(int)*k);
    que->head = 0;
    que->tail = 0;
    // 数组长度减去k再加1就是输出数组的长度了
    *returnSize = numsSize-k+1;
    int* res = (int*)malloc(sizeof(int)*(numsSize));
    int top = 0;
    // 先把前k个数字添加进队列
    for(int i=0; i<k; i++){
        QueuePush(que, nums[i], k);
    }
    // res记录第一个窗口的最大值
    res[top++] = QueueFront(que, k);
    for(int i=k; i<numsSize; i++){
        // 对窗口最左边要移除的元素进行pop
        QueuePop(que, nums[i-k], k);
        // 对窗口最右边要进来的元素进行push
        QueuePush(que, nums[i], k);
        // res记录当前窗口的最大值
        res[top++] = QueueFront(que, k);
