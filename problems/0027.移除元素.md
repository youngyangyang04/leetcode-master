...
双指针法二：**如果要删除的元素比较少的时候**

还有一种双指针适合要删除的元素比较少的时候。比如对 `nums=[1,2,3,5,4],val=4` 来说，第一种双指针会有一些不必要的移动操作。对这种情况，当遇到 `nums[i]==val` 的时候，可以将当前元素于最后一个元素交换，同时将数组个数减 1。需要注意的是，最后一个元素仍然可能是需要删除的。对此不用担心，下一次检查的时候仍然会检查这个元素，从而可以将其删掉。
...
- 时间复杂度：$O(n)$ 如果要删除的元素较少，其交换操作数等于要删除的元素个数，比第一种双指针更高效
- 空间复杂度：$O(1)$
