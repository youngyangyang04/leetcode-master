...
class Solution(object):
    def longestPalindrome(self, s="babad"):
        """
        :type s: str
        :rtype: str
        """
        ...
        for i in range(len(s)):
            ...
            maxlength, left, right = self.extend(s,i,i,len(s))
            if maxlength not in result:
                result[maxlength] = [left,right]
            maxlength, left, right = self.extend(s,i,i+1,len(s))
            if maxlength not in result:
                result[maxlength] = [left, right]
        maxlength = max(result.keys())
        return s[result[maxlength][0]:result[maxlength][1]]
    ...
    def extend(self, s, i, j, n):
        ...
        while (i>=0 and j <n and s[i]==s[j]):
            if maxlength<len(s[i:j+1]):
                maxlength=len(s[i:j+1])
                left = i
                right = j+1
            i -=1
            j +=1
        return maxlength, left, right
 ```
