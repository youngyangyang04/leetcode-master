...
遍历背包时候的逆序有点难理解，我们可以换个思路。

其实，我们可以先用0号物品来初始化dp数组，和二维dp时候对第一行(也就是物品0)的dp初始化一样
...
总的来说，遍历当前物品i得到的dp数组，就是二维dp数组里对应的第i行
...
void test_1_wei_bag_problem() {
    ...
    for (int i = 1; i <= bagWeight; i ++) { // 0号物品初始化dp数组
        if (weight[0] > i) {
            continue;
        }
        dp[i] = value[0];
    }
    for (int i = 1; i < weight.size(); i ++) { // 遍历物品 从1号物品开始
