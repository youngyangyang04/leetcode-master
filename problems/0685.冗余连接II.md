...
/**
 * 思路：一共有两种情况。
 * 一是无环时（也就是说会有一个节点有两个父亲），这时候，有两个父亲的那个节点，对应的那两条边，
 *  肯定有一个是答案，这时候我们就看现在输入的样例，如果他们在join()前就已经连通了，
 *  就说明该组样例肯定是答案，否则，另一条边就是答案。
 * 二是有环时，就退化成冗余连接。
 */
public class Main {
    static int[] fa = new int[1005];// fa[i] : i节点的父亲是fa[i]
    static List<Integer>[] li = new List[1105]; //记录有几个节点指向i节点
    ...
    public static void main(String[] args) {
        ...
        for (int i = 1; i <= n; i++) {
            ...
            if (li[t2].size() == 2) {
                //不加之前就连通了，就删它
                if (fin(t2) == fin(li[t2].get(1))) { 
                    System.out.println(li[t2].get(1) + " " + t2);
                } else {//反之，肯定删这条边
                    System.out.println(li[t2].get(0) + " " + t2);
                }
                mark = 1;
                //防止输出两份代码
                continue; 
            }
            // 说明已经连通,t1,t2再相连就会形成环
            if (fin(t1) == fin(t2)) {
                mark = 1;
