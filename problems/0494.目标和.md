...
很不幸，leetcode修改了测试用例，导致上述代码在执行时，报错：
```
...
class Solution {
    public int findTargetSumWays(int[] nums, int target) {
        ...
        //如果(sum - target)除以2的余数不为0，也是没有方案的
...
class Solution {
    public int findTargetSumWays(int[] nums, int target) {
        ...
        // 利用二元一次方程组将right用target和sum表示出来（替换掉left组合），详见代码随想录对此题的分析
        // 如果所求的right数组和为大数，则作为整数数组的nums里的任何元素自然是没有办法凑出这个大数的
        if((sum - target) % 2 != 0) {
            return 0;
        }
        ...
        // dp[i][j]：遍历到数组第i个数时， right为j时的能装满背包的方法总数
...
class Solution:
    ...
    def findTargetSumWays(self, nums: List[int], target: int) -> int:
        ...
        bagSize = (total - target) // 2  # 转化为组合总和问题，bagSize就是目标和
...
class Solution:
    def findTargetSumWays(self, nums: List[int], target: int) -> int:
        ...
        if (total_sum - target) % 2 == 1:
            return 0  # 此时没有方案
        target_sum = (total_sum - target) // 2  # 目标和
...
function findTargetSumWays(nums: number[], target: number): number {
    ...
    const right: number = (sum - target) / 2;
    
    // 将问题转化为装满容量为left的背包有多少种方法
    // dp[i]表示装满容量为i的背包有多少种方法
    const dp: number[] = new Array(right + 1).fill(0);
    dp[0] = 1;  // 装满容量为0的背包有1种方法（什么也不装）
    for (let i: number = 0; i < nums.length; i++) {
        for (let j: number = right; j >= nums[i]; j--) {
            dp[j] += dp[j - nums[i]];
        }
    }
    return dp[right];
