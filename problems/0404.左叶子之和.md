...
```java
class Solution {
    // 通过添加一个标志位，可以递归到叶子节点，而不是提前判断左叶子节点
    public int sumOfLeftLeaves(TreeNode root) {
        return sumOfLeftLeavesHelper(root, false);
    }
    // isLeftChild表示当前节点是否是左孩子
    private int sumOfLeftLeavesHelper(TreeNode node, boolean isLeftChild) {
        if (node == null) {
            return 0;
        }
        // 如果当前节点是叶子节点并且是左孩子，则返回其值
        if (isLeafNode(node) && isLeftChild) {
            return node.val;
        }
        // 递归计算左右子树中的左叶子节点之和
        int leftSum = sumOfLeftLeavesHelper(node.left, true);  // 将左孩子标记为true
        int rightSum = sumOfLeftLeavesHelper(node.right, false); // 右孩子标记为false
