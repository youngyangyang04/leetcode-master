...
其实本题思路和标准的回溯法一模一样，只要我们对其做一下分析即可：题目可以理解为在一个9 * 9（纵向）的格子内，每个格子尝试放入 1 ~ 9（横向）中的一个数字，使得满足要求的组合有多少个。因此，很快就可以写出下面的代码（JavaScript）：

``` javascript
function sudoku(board) {
  // 这里board即是输入，也是临时数据存储区
  ...
  function isValid(row, col, n) {
    // 这里以最朴素的方式来进行检查
    // 也可以通过以空间换时间的方式来进行加速，基本思想是在全局维护各行，各列，各小三角3种集合，以Set作为数据结构存储，快速判断是否有重复
  ...
  function backtrack(pos) {
    // 终止条件，所有满足条件的格子都已经填满了
    if (pos === total) {
      // 如果这里对最终的board有要求，比如横、竖、斜相加都相等，那么可以在此排除掉部分结果
      result.push(board.map((v) => [...v]));
      return;
    }

    // 通过格子位置，计算目前是第几行，第几列
    const row = Math.floor(pos / colCount);
    const col = pos - row * colCount;
...
如果只要找第一个，那么只需要将`backtrack`方法在`pos === total`的时候返回一个状态即可，在`for`循环内调用时，判断返回值，如果是状态值，那么直接将其返回就可以。
