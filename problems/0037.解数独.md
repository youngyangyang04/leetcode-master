...
当然，如果单层递归的思维根深蒂固，也可以先把“**二维降为一维**”，即只关注需要处理的点“.”，把它们的位置`(i, j)`放到一维数组中，再用常规的递归加回溯解法，详解见`Python 解法`。
...
class Solution:
    ...
    def solveSudoku(self, board: List[List[str]]) -> None:
        self.initialize_points(board) # 二维降为一维，记录所有需要处理的点

        self.backtracking(board)

    def backtracking(self, board):
        for point in self.points: # 只关注需要处理的点
            row = point[0]
            col = point[1]

            if board[row][col] != '.': # 之前处理过了，跳过
                continue

            for num in range(1, 10): # 横向遍历
                if not self.is_valid(board, str(num), row, col):
                    continue
                
                board[row][col] = str(num) 
                if self.backtracking(board): # 纵向遍历
                    return True # 向上冒泡，直到顶层，直接结束程序
                board[row][col] = '.' # 回溯
            
            return False # 未发现合适的数字，返回 False 将导致回溯
        
        return True # 所有点都处理完了，返回 True
    ...
    def is_valid(self, board, num, row, col):
        ...
        return num not in sub_box_nums

    def initialize_points(self, board):
