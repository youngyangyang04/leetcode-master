...
[N 皇后问题](https://programmercarl.com/0051.N皇后.html)是因为每一行每一列只放一个皇后，只需要一层 for 循环遍历一行，递归来遍历列，然后一行一列确定皇后的唯一位置。
...
在校验一个值能否使用的时候，每次遍历当前行、列和九宫格所需的总时间也是比较久的。这个校验的过程是可以优化的，我们在填充 `board[i][j] = n`的时候，可以使用二维数组 row、column、三维数组 block 来记录该值已使用过，这样后续检查的时候直接访问对应位置的值，就可以知道是否合法了，但是在一个值不合法，将`broad[i][j]`重置为'.'时，要同时清除该值在该位置的使用记录。

```Javascript
const isValid = (i, j, num, row, column, block) => {
    return (
        !row[i][num] &&
        !column[j][num] &&
        !block[Math.floor(i / 3)][Math.floor(j / 3)][num]
    );
};
const set = (i, j, num, row, column, block, value) => {
    row[i][num] =
        column[j][num] =
        block[Math.floor(i / 3)][Math.floor(j / 3)][num] =
            value;
...
/**
 * 优化思路，在遍历时，存储一个数字在当前行，当前列，当前九宫格中是否出现过，下次不再次遍历了
 * @param {character[][]} board
 * @return {void} Do not return anything, modify board in-place instead.
 */
const solveSudoku = function (board) {
    //solve(i) 代表填充第几个空白格，如果能正确填充，返回true，否则返回false
    function solve(i) {
        //base case ，全部填充完毕，返回true；
        if (i === spaces.length) {
            return true;
        }
        ...
        for (let num = 1; num <= 9; num++) {
            if (check(row, col, num, row, column, block)) {
                board[row][col] = String(num);
                //记录该值在当前行、当前列、当前九宫格都使用过
                set(row, col, String(num), row, column, block, 1);
                ...
                board[row][col] = '.';
                //移除使用记录
                set(row, col, String(num), row, column, block, 0);
            }
        }
        return valid;
    }
    //记录当前行是否使用过 row[0][1]===1 代表第0行 数字1 存在
    const row = Array(9)
        .fill(0)
        .map(() => Array(10));
    //记录当前列
    ...
    for (let i = 0; i < board.length; i++) {
        for (let j = 0; j < board[0].length; j++) {
            if (board[i][j] === '.') {
                ...
                set(i, j, board[i][j], row, column, block, 1);
            }
        }
    }
