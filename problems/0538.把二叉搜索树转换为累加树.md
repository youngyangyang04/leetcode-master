...
class Solution:
    ...
    def convertBST(self, root: Optional[TreeNode]) -> Optional[TreeNode]:
        ...
        while stack:
            ...
            if visited: # visited 为 True，可以收割
                self.sum += node.val
                node.val = self.sum
                continue

            # visited 为 False, 表示初次访问，目的是“把自己和两个儿子在栈中放对位次”，尚不能收割
            if node.left: # 左儿子先入后出
                stack.append((node.left, False))

            stack.append((node, True)) # 中间自己，再次入栈，但 visited 设置为 True

            if node.right:
                stack.append((node.right, False)) # 右儿子后入先出
