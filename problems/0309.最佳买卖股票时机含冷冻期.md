...
如果第i天持有股票即dp[i][0]， 那么可以由两个状态推出来，取二者最大值

- 第i-1天就持有股票，那么就保持现状，所得现金就是昨天持有股票的所得现金 即：`dp[i][0] = dp[i - 1][0]`
- 第i天买入股票，所得现金就是**前天不持有股票**的所得现金 减去 今天的股票价格 即：`dp[i][0] = dp[i - 2][1] - prices[i]`

根据题目冷冻期定义“卖出股票后，你无法在第二天**买入**股票 (即冷冻期为 1 天)”，**如果想要第i天买入股票，在第i-2天的交易状态必须为未持有**。

所得现金就是**前天**不持有股票的所得最大现金 减去 今天的股票价格。

再来看看如果第i天不持有股票即dp[i][1]的情况， 依然可以由两个状态推出来

- 第i-1天就不持有股票，那么就保持现状，所得现金就是昨天不持有股票的所得现金 即：`dp[i][1] = dp[i - 1][1]`
- 第i天卖出股票，**冷冻期对卖出股票没有影响**，所得现金就是按照今天股票价格卖出后所得现金即：`dp[i][1] = dp[i - 1][0] + prices[i]`
...
由递推公式看出，至少前2天需要初始化，因为第3天开始才需要考虑冷冻期
...
`dp[1][0]`表示第1天持有股票，此时的持有股票就一定是买入股票了，因为有冷冻期限制，要么第0天买入、要么第1天买入，所以`dp[1][0] = max(-prices[0], -prices[1])`

`dp[1][1]`表示第1天不持有股票，可能是第0天买入第1天卖出，也可能是第0天第1天都没买入，所以`dp[1][1] = max(prices[1] - prices[0], 0)`

由于利润不可能为负数，dp数组其他非0下标初始化为0就好

4. 确定遍历顺序

从递推公式可以看出dp[i]都是由dp[i - 1]、dp[i - 2]推导出来的，那么一定是从前向后遍历。
