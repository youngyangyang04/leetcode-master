...
```
另一种思路，因为不能再卖完以后的第二天进行购买，所以我们在Leetcode122的动规方程可以改变一下：
dp[i][0] = max(dp[i-1][0], dp[i-2][1]-prices[i]) 即购买的前提是大前天卖了以后获得的最多情况下。别的细节并没有任何区别。
因为涉及到的是前两天的事情，所以我们初始化的时候需要将前两天的都初始化。
