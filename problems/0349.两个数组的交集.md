...
方法二
- 定义hash表为元素存在表，存在为1，不存在为0
- 遍历num1：num1的元素在hash表中置为1
- 遍历num2：num2的元素且在hash表中已存在（对应值为1），则该元素加入ans数组
```C
int* intersection(int* nums1, int nums1Size, int* nums2, int nums2Size, int* returnSize) {
    // 初始化
    int *ans = (int*) malloc(sizeof(int) * 1000);
    *returnSize = 0;
    int table[1000] = {}; // hash表
    // 遍历nums1
    for (int i = 0; i < nums1Size; i++)
    {
        int a = nums1[i];
        if (table[a] == 0) // 未出现的元素才加入hash表
            table[a]++;
    }
    // 遍历nums2
    for (int i = 0; i < nums2Size; i++)
    {
        ...
        {
            ans[(*returnSize)++] = a;
            table[a]++; // 存在性置为2，避免nums2中的重复元素再加入ans
        }
    }
    return ans;
}
