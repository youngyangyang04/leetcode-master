...
class Solution {
    ...
    public int[] intersection(int[] nums1, int[] nums2) {
        ...
        for(int i = 0; i < nums1.length; i++){//Set特点:无序，不重复
            nums1Set.add(nums1[i]);
        }
        for(int i = 0; i < nums2.length; i++){
            if(nums1Set.contains(nums2[i])){//判断2中的数字是否在1中出现过
        ...
        int[] res1 = new int[nums2Set.size()];
        index = 0;
        //2迭代器法
        Iterator<Integer> it = nums2Set.iterator();
        while(it.hasNext()){
            res1[index++] = it.next();
        }
        return res;
