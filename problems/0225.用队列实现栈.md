...
如下面动画所示，**用两个队列in和out实现队列的功能，out其实完全就是一个备份的作用**，把in队列最后面的元素以外的元素都备份到out 队列，然后弹出最后面的元素，再把其他元素从out队列导回in队列。
...
class MyStack {
 ...
 public:
  ...
  int pop() {
    ...
    // 这个时候 in 队列只剩下一个元素
    int element = in.front();
    in.pop();
    in.swap(out);  // in 队列为空, 使用 swap 和out交换
    // 相同逻辑
    // in = out;
    // while (!out.empty()) {
    //   out.pop();
