...
function allPathsSourceTarget(graph: number[][]): number[][] {
    ...
    const dfs = (graph: number[][], start: number): void => {
        // 结束递归（遇到n-1节点）
        ...
        for (let n = 0; n < graph[start].length; n++) {
            path.push(graph[start][n]);
            dfs(graph, graph[start][n]);
            path.pop();
        }
    }
...
>>>>>>> 897a31c80184ebf524217846256b2feb44449b61
<p align="center">
