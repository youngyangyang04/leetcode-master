...
int minDepth(struct TreeNode* root) {
    ...
    int q_len = 100;
    struct TreeNode **q = (struct TreeNode **)malloc(q_len * sizeof(struct TreeNode *));
    assert(q);

    int head = 0, tail = 0;
    q[tail++] = root;

    int level_size, depth = 0;
    while (level_size = tail - head) {
        for (int i = 0; i < level_size; i++) {
            struct TreeNode *temp = q[i + head];
            ...
            if (tail + 2 >= q_len) {
                q_len *= 2;
                q = (struct TreeNode **)realloc(q, q_len * sizeof(struct TreeNode *));
                assert(q);
            }
