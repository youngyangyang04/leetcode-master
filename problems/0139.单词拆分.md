  ...
  bool wordBreak(string s, vector<string>& wordDict) {
    ...
    for (int i = 1; i <= s.size(); ++i) {  // 遍历背包
      ...
      for (int j = 0; j < wordDict.size(); ++j) {  // 遍历物品
        if (i < wordDict[j].size()) continue;  // 背包容量小于当前物体重量
        string word =
            temp.substr(temp.size() - wordDict[j].size(), wordDict[j].size());
        if (word == wordDict[j] && dp[i - wordDict[j].size()]) dp[i] = true;
      }
    }
    return dp[s.size()];
  }
...
* 时间复杂度：O(n*m)（n为string的长度,m为"物品"word数量）
* 空间复杂度：O(n)
