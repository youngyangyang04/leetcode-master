...
数学上也是可以证明的，一个简单验证的实例是，记环形起点为0，环有n个节点，index记录为0~(n-1)，设slow刚进入环在index=0处时，fast在环中index=x处（0<=x<n），经过n-1-x个step后，slow达到index=n-x-1处的节点，fast达到index=x+2(n-1-x)=2n-2-x，fast实际已经绕环一周，实际fast的index=2n-2-x-(n-1)=n-1-x，此时fast和slow的位置重合。
