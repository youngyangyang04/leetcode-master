...
在目前方法的基础上，还可以有一个小小的改进，不过算法效率并没有提升，所以不感兴趣的话，到这里就可以啦。

当前我们会计算[1, n]每个数字作为根节点时，所有搜索二叉树的组合，在计算组合时，会有一定的重复，我们可以简化这部分的计算。

以n = 7为例，如下展示了计算过程中所得到的组合数：

```java
3 // 本轮节点个数为 3
2 1 2  // 从左至右分别表示，当1、2、3作为根节点时，所能得到的搜索树的个数
...
可以看到每轮计算的时候，都有一倍的重复量，减去这部分重复计算后，代码如下：
...
class Solution {
    public int numTrees(int n) {
        ...
        for (int i = 2; i <= n; i++) {
            int loops = i / 2;
            if (i % 2 == 1) {
                // 如果当前节点个数为奇数，需要加上当左右子树数目相同时的组合情况
                dp[i] += dp[loops] * dp[loops]; 
            }
