...
impl Solution {
    /// 前序遍历
    fn preorder_tranversal(root: Option<Rc<RefCell<TreeNode>>>) -> Vec<i32> {
        if root.is_none() {
            return vec![];
        }
        ...
        fn tranversal(root: Option<Rc<RefCell<TreeNode>>>, res: &mut Vec<i32>) {
            if let Some(node) = root {
                res.push(node.borrow().val);
                tranversal(node.borrow_mut().left.take(), res);
                tranversal(node.borrow_mut().right.take(), res);
            }
        }

        tranversal(root, &mut res);

        res
    }

    /// 中序遍历
    fn inorder_tranversal(root: Option<Rc<RefCell<TreeNode>>>) -> Vec<i32> {
        if root.is_none() {
            return vec![];
        }
    ...
    /// 后序遍历
    fn postorder_tranversal(root: Option<Rc<RefCell<TreeNode>>>) -> Vec<i32> {
        if root.is_none() {
            return vec![];
        }
