...
类似题目[27. 移除元素](https://www.programmercarl.com/0027.移除元素.html)，使用双指针的方法，可以将额外空间复杂度降低至 O(1)，具体实现是一个 top 指针表示栈顶位置，一个 i 指针表示当前处理到的位置，C++代码如下：
...
class Solution {
  public:
    int evalRPN(vector<string> &tokens) {
        ...
        return stoi(tokens[top - 1]);
    }
};
```

代码里使用了 to_string (int val) 和 stoi (const string&  str) 函数，to_string() 时间复杂度 O(log(m))，m为val最大值，stoi() 时间复杂度 O(n)，n为字符串长度，本题处理对象都是32位int范围内的整数或者对应字符串，可以认为两个函数都是 O(1) 内完成的，因此时间复杂度 O(n)，额外空间复杂度 O(1)
