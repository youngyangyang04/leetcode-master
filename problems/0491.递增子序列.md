...
```Java
class Solution {
    //依照惯例进行initialization
    List<List<Integer>> res = new ArrayList<>();
    List<Integer> cur = new ArrayList<>();
    public List<List<Integer>> findSubsequences(int[] nums) {
        //进行回溯，初始low设为minvalue
        backtracking(0, nums, Integer.MIN_VALUE);
        return res;
    }
    private void backtracking(int start, int[] nums,int low){
        //判断条件，如果两个及以上就可以加入res
        if (cur.size() >= 2){res.add(new ArrayList<>(cur));}
        //可在这里加入return判断，不加入的话也无妨，下面的循环也可以进行判断
        // if (start == nums.length) return;

        //重点，同一层使用一个hashset保证同一层不会加入两个一样的数
        Set<Integer> set = new HashSet<>();
        for (int i = start; i < nums.length; i++){
            //如果遇到重复的的数，或者遇到小于当前（low）的数直接跳过
            if (set.contains(nums[i]) || nums[i] < low){continue;}
            set.add(nums[i]);
            cur.add(nums[i]);
            //更新初始坐标（i+1）和当前最小满足条件数值（low）
            backtracking(i + 1, nums, nums[i]);
